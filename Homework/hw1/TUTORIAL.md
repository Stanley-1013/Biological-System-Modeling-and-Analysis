...
幾年後，科學家登島調查，發現已有蕨類、蜘蛛和鳥類回到島上。數十年間，生物多樣性逐漸恢復。生態學家因此有了一個難得的機會：**從零開始觀察一個島嶼上的物種如何累積**。這個自然實驗，直接啟發了後來的島嶼生物地理學理論。
...
這是一個**穩定的均衡**——就像鐘擺在最低點一樣，被擾動後會自動回來。
...
- **棲地破碎化**：農田開發把森林切割成許多小「島嶼」，每塊面積縮小，物種平衡點 $R^*$ 降低，長期來看物種會慢慢滅絕
...
**物理意義**：
- 截距 $a_I \approx 6.193$：空島（$R=0$）時的移入率，每年約 6.2 個物種
- 斜率 $b_I \approx -0.01132$：每增加一個物種，移入率平均下降 0.011 個物種/年
...
> **注意**：$R^2 = 0.268$ 偏低，表示線性模型對移入率的配適度有限——資料散佈很大。這是現實生態資料的常見狀況。相比之下，滅絕率的 $R^2 = 0.860$ 就很不錯。
...
| $\Delta t$ | 效果 |
|:----------:|------|
| 太大（如 $\Delta t = 100$ 年）| 可能發生數值振盪，甚至發散（不穩定）|
...
**生物直覺**：當島上已有 $R$ 個物種，每個新移入者成功定殖的機率大約是 $e^{-\alpha R}$——棲位愈飽和，成功率以指數速率下降。
