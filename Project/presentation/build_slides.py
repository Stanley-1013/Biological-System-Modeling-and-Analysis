...
NOTES = {
    1: (
        "各位同學、老師好，我是李傳漢，學號 B11611027。今天我要報告的是 term project "
        "的 Part 2：在 sIC AIDS model 上面，加進 HIV vaccination。\n\n"
        "這個 model 來自 Haefner 2005 課本第 15 章。它其實是 Imperial College model "
        "的簡化版，我們就叫它 sIC。那 Part 2 想回答的，是一個很實際的問題：打疫苗，到底"
        "能不能讓 HIV 疫情往下走？要花多少錢？最好的接種率又是多少？還有，跟推廣保險套"
        "比起來，哪一個比較有效？\n\n"
        "接下來這十五分鐘，我會先把 model 講清楚，再一題一題回答這四個問題。"
    ),
    2: (
        "先講一下，為什麼要用 model。\n\n"
        "HIV 是透過性行為傳播的，而且它拖很久，疫情是用「幾十年」當單位在跑的。這種時間"
        "尺度，光靠直覺其實很不可靠。你很難憑感覺去判斷，一個介入措施在二、三十年後，會"
        "累積出什麼樣的結果。\n\n"
        "所以我們用 compartment model，把一個原本很模糊的政策問題，變成可以量化、可以"
        "在同一個 baseline 上面公平比較的問題。\n\n"
        "那 Part 2 具體就是四個問題：(a) 打疫苗會不會讓疫情先到高峰、再下降；(b) 每避免"
        "一個感染，要花多少錢；(c) 最好的接種率是多少；還有 (d) 疫苗跟保險套，到底哪個"
        "比較好。這四題，我都會回到同一個 baseline 來比。"
    ),
    3: (
        "好，先來看 model 本身。\n\n"
        "sIC 把人口切成 12 個 compartment。先是三個疾病狀態：S 是 susceptible、易感者；"
        "I 是感染了 HIV、但還沒發病；A 是臨床 AIDS。這三個再乘上性別，女是 f、男是 m；"
        "再乘上兩個年齡層，age 1 是 0 到 15 歲、還沒有性行為，age 2 是 16 歲以上、有性"
        "行為。三乘二乘二，剛好 12 格，就是右邊這張表。\n\n"
        "這裡有幾個重點。第一，只有 age 2 的人會傳染。第二，這是一個 Forrester 式的 "
        "flow model，裡面有出生、有 ageing 用 ξ、有 I 進展到 A 用 γ、還有 AIDS 死亡 α。\n\n"
        "那最關鍵的，是 force of infection。HIV 是 frequency-dependent。意思是說，重要"
        "的不是感染者的「絕對數量」，而是在你可能的性伴侶裡面，有多大「比例」是感染者。"
        "所以是 I 除以 (S+I)。注意一下，A 不在分母裡面，因為我們假設臨床 AIDS 的人不再"
        "有性行為了。這個細節，等一下會變得很重要。"
    ),
    4: (
        "這張是 Table 15.2 的關鍵參數。我想特別點一件事：傳染機率是不對稱的。\n\n"
        "男傳女，β 是 0.20；女傳男，只有 0.075，差不多 2.7 倍。這是異性戀 HIV 一個很"
        "標準的生物特性，也正是為什麼在 baseline 裡面，女性的 equilibrium prevalence "
        "會比男性高。\n\n"
        "人口結構這邊，也有幾個要記得的點。出生只由 age-2 的女性產生，而且發展中國家生育"
        "率高，所以人口在前期是會成長的。另外還有一條 perinatal route，ϑ 等於 0.35，意思"
        "是感染的母親，有機會生出已經感染的新生兒。這條傳染路徑，疫苗是擋不住的，等一下"
        "講 threshold 的時候會再提到。初始人口 N(0)，是 8005 人。"
    ),
    5: (
        "接下來這兩張，是我為了讓 model 真的跑出課本描述的行為，必須做的兩個判斷。我選擇"
        "老實講出來，因為它們會影響結果。\n\n"
        "第一個，是進展率 γ。R₀ 的公式，是 c 乘上根號 (β_mf × β_fm)，再除以 (μ+γ)。"
        "課本 Table 15.2 給的 γ 是 1.16，可是這代表從感染到發病，平均只有 0.85 年、"
        "不到一年。這跟課本自己講的「1 到 10 年」是矛盾的。更糟的是，把 1.16 代進去，"
        "R₀ 大概只有 0.24，小於 1，疫情根本點不起來。我實際去模擬，seed 進去的感染就"
        "直接消失了。這跟 Fig 15.5「HIV 持續存在」完全相反。\n\n"
        "所以我改用比較有生物根據的 γ，等於 0.1，平均感染期大概 8.1 年，落在課本講的範圍"
        "裡面。這時候 R₀ 大概是 2.35，大於 1，疫情就會 invade，跟課本的圖一致。對應的 "
        "herd-immunity threshold，也就是 p_c，是 1 減 1 除以 R₀，大約 0.574。右邊這"
        "張圖，就是修正之後的 baseline：HIV 持續存在，而且女性 prevalence 高於男性。"
    ),
    6: (
        "第二個判斷，是 seed 要放哪裡。\n\n"
        "Table 15.2 是放 5 個 AIDS 男性，A_{m2} 等於 5。可是前面講過，A 不在 force "
        "of infection 裡面，所以這 5 個人根本傳不出去。他們只會進展、死亡，疫情永遠不會"
        "開始，我模擬出來 incidence 剛好就是零。那最小的修正，就是改成 seed 5 個 "
        "infectious 男性，I_{m2} 等於 5。\n\n"
        ...
        "β，減半剛好讓 R₀ 變成 1.17，只比 threshold 高一點點，所以 prevalence 就崩掉"
        "了。這就是 near-threshold 的敏感性：在 1 附近，小小的調整就會讓結果差很多。"
        "這邊我選擇把這個 divergence 講出來，而不是去硬調參數。"
    ),
    7: (
        "好，現在來加上疫苗。\n\n"
        "我加了兩個 protected compartment，P_{f2} 跟 P_{m2}，只在 age 2。易感者會以 "
        "per-capita 速率 ν 被接種、進到 P，而保護力會以速率 l 衰退、再回到 S。所以它是"
        "一個 S 到 P、再回到 S 的循環。\n\n"
        "疫苗型態，我用的是 take，也就是 all-or-nothing：在 P 裡面的人完全受保護，force "
        "of infection 不會作用在他們身上。專案的標準值是 ν 等於 0.65、l 等於 0.1。\n\n"
        "那這裡有一個解析上的上限，叫 waning ceiling：P 佔 (S+P) 的比例，最多到 ν 除以 "
        "(ν+l)，也就是 0.65 除以 0.75，大約 0.87。所以就算你一直打，任何一個時刻，最多"
        "也只有大約 87% 的人受保護。成本我用一個 accumulator 去累計，dV/dt 等於 ν 乘上"
        "兩性 age-2 的易感者，每劑算 10 美元。\n\n"
        "那最關鍵的一個慣例，在最後一點。受保護的人雖然沒被感染，可是他們還是有性行為、"
        "還是合法的性伴侶，只是跟他們發生關係不會傳染。所以他們要留在分母裡面，force "
        "of infection 就變成 I 除以 (S+I+P)。把易感者移進 P，會稀釋掉感染者的比例。而"
        "這個稀釋，其實正是 herd immunity 的機制。"
    ),
    8: (
        "那第一題：打疫苗，會不會讓疫情先到高峰、再下降？答案其實比「高峰再下降」還要"
        "更強。\n\n"
        ...
        "5 個 seed 男性傳給少數幾個人，之後就單調往下、掉到接近零。對比 baseline，它衝到"
        "大約每年 791 個新感染，在第 48 年左右到高峰，而且維持得很高。那這個下降不是瞬間"
        "發生的，因為原本的 I 跟 A，還要花八到十年才會進展、死亡。但因為 R_eff 小於 1，"
        "沒有新的傳染去補充，病毒就被清掉了。同時 protected fraction 飽和在大約 0.85，"
        "剛好在 0.867 這個上限下面一點點，符合 take-with-waning 的穩態。"
    ),
    9: (
        "第二題，是成本。\n\n"
        "用剛剛那個 cost accumulator，右邊這張表列了幾個時間點。20 年累積花大約 11 萬 "
        "4 千美元，30 年大約 16 萬 2 千，50 年大約 24 萬 7 千，大致上是線性成長。\n\n"
        "那重點是，cost-effectiveness 會隨時間「明顯變好」。每避免一個感染的成本：20 年"
        "的時候是 372 美元，30 年降到 102 美元，50 年只剩 18 美元。原因是，早期 baseline "
        "疫情還沒起來，能避免的感染很少，所以看起來貴；等到 baseline 加速、衝向每年 791 "
        "的高峰，而疫苗這邊一直接近零，avoided infections 增加得比成本快很多，單價就"
        "一路往下掉。\n\n"
        "30 年的 102 美元，落在 Stover 跟 Garnett 2002 報告的範圍裡面，算合理。另外，"
        "飽和之後，疫苗會因為保護力衰退、加上新人長大，持續以大約每年 407 劑、約 4070 "
        "美元的速率 re-vaccinate。這邊我要強調一下：因為這是一個很小的合成人口，能搬到"
        "別處去用的指標，是「每避免一個感染的成本」，而不是總金額。"
    ),
    10: (
        "第三題：最佳接種率。先講 epidemiological threshold，也就是最小、能把 R_eff "
        "壓到 1 以下的 ν_c。\n\n"
        "條件是，穩態的 protected fraction、也就是 ν 除以 (ν+l)，要大於等於 p_c。解出"
        "來的 closed form，是 ν_c 等於 l 乘上 p_c 除以 (1 − p_c)，大約 0.135。可是這"
        "只是一個 lower bound，因為它忽略了兩件事：一個是 perinatal、那條疫苗擋不住的"
        "傳染路徑，另一個是 mortality 也會作用在 P 上面。\n\n"
        "所以我就直接用 invasion test 去量。做法是，在打過疫苗的無病人口裡，丟一點點感染，"
        "看早期成長率 r 在哪個 ν 穿過零、也就是 R_eff 等於 1。模擬出來的 ν_c 大約是 "
        "0.366，右邊這張圖，就是那個 zero crossing。標準的 ν 等於 0.65，舒服地在它上面，"
        "所以確實能控制疫情。\n\n"
        "最後一點 caveat。如果你錯誤地把 P 排除在分母外面，這個 threshold 會整個消失。"
        "但那是分母選擇造成的 artifact，不是真實的結果。所以我特別把這個對照，也畫進去了。"
    ),
    11: (
        "這張一樣是第三題，但「最佳」其實有兩個合理的意思，我兩個都報。前一張是 "
        "epidemiological 的；這一張，是 cost-effective 的。\n\n"
        "關鍵在於：avoided infections 在超過 ν_c 之後，會「飽和」。在 ν_c 以下，每多"
        "一點 ν，都能多避免很多感染；可是一旦超過 ν_c，大約 13400 個可避免的感染，幾乎"
        "都已經被避免掉了。這時候你再加大接種，其實只是在重複幫衰退的人補打，效益很低。\n\n"
        "所以這裡有一個 diminishing-returns 的膝點。能抓到 99% 最大避免量的最小 ν，大約"
        "在 0.175，那邊的邊際成本，大約是每個感染 122 美元。標準的 ν 等於 0.65，雖然確實"
        "能控制疫情，但相對於這個 cost-effective 膝點，其實是「打多了」。這就是為什麼 "
        "epidemiological 最佳跟 cost-effective 最佳，答案會不一樣。"
    ),
    12: (
        "最後一題：疫苗跟保險套，哪一個比較會控制疫情？\n\n"
        "其實，兩個都能控制。疫苗把 R_eff 壓到 0.31，嚴格小於 1；保險套把 R₀ 降到 1.17，"
        "還是稍微在 1 上面。也就是說，減半 β 在這個 R₀ 之下，還差一點點才跨過 threshold，"
        "但疫情已經慢到，在我們的時間範圍內等於被控制住了。\n\n"
        "在 avoided infections 上面，兩者幾乎一樣。總共 31235 個感染，疫苗避免 31224、"
        "保險套避免 30857。疫苗只是稍微領先，因為它讓 R_eff 嚴格小於 1，殘留 11 個感染；"
        "而保險套，殘留 378 個。\n\n"
        "那真正差很多的，是成本基礎，這也是為什麼我不願意硬選一個贏家。疫苗有明確的 34 萬 "
        "8618 美元，而且因為 waning ceiling，要無限期地幫衰退的人補打。保險套這邊，是被"
        "當成永久把 R₀ 砍半、而且沒有定價。但「沒定價」不等於「免費」，真實的行為改變計畫，"
        "也有它自己的預算，只是專案沒給。再加上 near-threshold 的敏感性，兩邊的結論其實都"
        "對 R₀ 很敏感。所以老實的講法是：在疫情結果上，是接近平手，疫苗因為嚴格跨過 "
    ...
    14: (
        ...
        "就會讓 endemic level 變很多。第三，partner-pool 慣例，也就是把 P 放進分母，"
        "這是 load-bearing 的，它正是疫苗 herd-immunity 效果的來源。第四，人口很小、"
        "是合成的，N₀ 只有 8005，所以金額是示意性的，要用每個感染的成本來看。第五，我只"
        "測了單一操作點，ν 等於 0.65、l 等於 0.1，而且把 ν 當成常數速率，這是對 "
